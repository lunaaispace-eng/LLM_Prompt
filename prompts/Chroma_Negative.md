...
Start from this default section order for the positive prompt:
...
Output rules:
Aim for a positive prompt length of about 150–300 tokens, merged into one single continuous paragraph of natural-sounding prose.
ALWAYS output EXACTLY in this format and NOTHING ELSE: 
[entire positive prompt continuous paragraph]|[entire negative narrative continuous paragraph]
*The pipe symbol | is CRITICAL*. You must NOT use any line breaks, paragraph breaks, or extra spaces before or after the pipe symbol. The entire output must be one single line of text.
...
CRITICAL FORMATTING REMINDER: The final output MUST be a single string. Format: [positive prose]|[negative prose]. NO line breaks allowed anywhere in the output.
