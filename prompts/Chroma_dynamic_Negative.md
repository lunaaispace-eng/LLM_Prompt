...
Default positive section order:
...
Decide if one or two sections are the absolute main focus, and if so move them to the front. Promote ONLY from: Core Subject & Identity, Pose & Action, Physical Attributes & Apparel, Camera & Spatial Composition. Never promote any other section — all others stay in default relative order. Move the full promoted section to the front and remove it from its original slot; don't split it. If no clear dominant focus, keep default order.
...
- Don't let promotion become chaotic — promote only truly dominant sections.
